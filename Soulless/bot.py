...
async def on_member_update(before, after): #for spam accounts joining in Soul Sanctum
    ...
    if len(sus_colors.intersection(roles_set)) > 1 and len(sus_clubs.intersection(roles_set)) > 2 and datetime.now(datetime_module.timezone.utc) - after.joined_at < timedelta(hours=1):
